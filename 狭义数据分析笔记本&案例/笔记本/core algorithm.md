...
在很多时候，我总是会把相关性分析和关联性分析的概念搞混，这里把他梳理一下

> 在数据分析和统计学中，关联性（Association）和相关性（Correlation）是两个不同的概念，它们用于描述不同类型的关系。
>
> 关联性（Association）指的是两个或多个变量之间的频繁共现关系。它衡量的是变量之间的联合出现的程度，即一个变量出现时，另一个变量也经常出现。关联性并不关心变量之间的因果关系，只关注它们之间是否存在某种模式或规律。关联性分析常用于市场篮子分析、推荐系统和协同过滤等领域。
>
> 相关性（Correlation）指的是两个变量之间的线性关系强度和方向的度量。它衡量的是变量之间的线性相关程度，即当一个变量发生变化时，另一个变量如何相应地变化。相关性的取值范围在-1到1之间，其中-1表示完全负相关，1表示完全正相关，0表示无相关性。相关性分析用于衡量变量之间的关联程度，并可以帮助我们理解变量之间的趋势和相互作用。
>
> 关联性和相关性之间的主要区别包括：
>
...
>
> 总之，关联性和相关性是用于描述数据中不同类型关系的概念。关联性关注变量之间的频繁共现关系，而相关性关注变量之间的线性关系强度和方向。在实际应用中，根据具体的问题和数据类型，可以选择适当的方法来分析关联性或相关性。
...
| 卡方检验 (Chi-Square Test)             | 卡方检验用于比较**两个或多个**`分类变量`之间的关联性。常用的卡方检验方法包括卡方独立性检验和卡方拟合度检验。 | 优点：适用于分类数据的统计推断，用于**检验观察频数与期望频数之间的差异**。缺点：对于样本量较小或期望频数较低的情况可能不准确。 |
